...
def register(request):
    if request.method == 'POST':
         ...
         user = User.objects.create(name=username,email=emailid,password=passwd)
         user.save()
...
def login(request):
    try:
        m = User.objects.get(email=request.POST.get('login_email'))
...
def home(request):
    try:
        person = User.objects.get(userid=request.session.get('user_id'))
